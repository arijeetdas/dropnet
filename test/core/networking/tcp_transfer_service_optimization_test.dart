...
void main() {
  ...
  group('TcpTransferService optimization smoke tests', () {
    ...
    test(
      'multi-file session asks approval once and transfers all files',
      () async {
        final sender = TcpTransferService();
        final receiver = TcpTransferService();
        ...
        expect(
          ...
          1,
          reason:
              'Batch transfer should require only one approval prompt per session.',
        );
        ...
        for (final file in sourceFiles) {
          ...
          expect(
            ...
            isTrue,
            reason: 'Missing received file: $name',
          );
          expect(
            await _sha(received),
            await _sha(file),
            reason: 'Checksum mismatch for $name',
          );
        }
    ...
    test(
      ...
      () async {
        ...
        final tempRoot = await Directory.systemTemp.createTemp(
          'dropnet_multi_target_',
        );
        final sourceDir = Directory(
          '${tempRoot.path}${Platform.pathSeparator}src',
        )..createSync(recursive: true);
        final dstA = Directory('${tempRoot.path}${Platform.pathSeparator}dstA')
          ..createSync(recursive: true);
        final dstB = Directory('${tempRoot.path}${Platform.pathSeparator}dstB')
          ..createSync(recursive: true);
        ...
        final totalBytes =
            files.fold<int>(0, (sum, file) => sum + file.lengthSync()) * 2;
        ...
        for (final file in files) {
          ...
          expect(
            await outA.exists(),
            isTrue,
            reason: 'receiver A missing $name',
          );
          expect(
            await outB.exists(),
            isTrue,
            reason: 'receiver B missing $name',
          );
          final sourceSha = await _sha(file);
          expect(
            await _sha(outA),
            sourceSha,
            reason: 'receiver A checksum mismatch for $name',
          );
          expect(
            await _sha(outB),
            sourceSha,
            reason: 'receiver B checksum mismatch for $name',
          );
        }
        ...
        expect(
          throughputMbPerSec,
          greaterThan(1.5),
          reason: 'Throughput unexpectedly low on localhost benchmark.',
        );
...
DeviceModel _device(String ip, String id, {String? tlsCertificateSha256}) {
  return DeviceModel(
    deviceId: id,
    deviceName: id,
    manufacturer: 'test',
...
Future<String> _localFingerprint() {

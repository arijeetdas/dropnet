...
class FtpSession {
  ...
  void processCommand(List<int> data) {
    try {
      _commandBuffer.write(utf8.decode(data, allowMalformed: true));
      var buffered = _commandBuffer.toString();
      final lines = buffered.split(RegExp(r'\r?\n'));

      final hasTerminated = buffered.endsWith('\n') || buffered.endsWith('\r');
      final completeLineCount = hasTerminated ? lines.length : lines.length - 1;

      for (var index = 0; index < completeLineCount; index++) {
        final commandLine = lines[index].trim();
      ...
      if (!hasTerminated && lines.isNotEmpty) {
  ...
  Future<String> _getIpAddress() async {
    try {
      final localAddress = controlSocket.address;
      if (localAddress.type == InternetAddressType.IPv4 &&
          !localAddress.isLoopback &&
  ...
  Future<void> listDirectory(String path) async {
    ...
    try {
      transferInProgress = true;
        ...
        final dirContents = await fileOperations.listEntries(path);
        logger.generalLog('Listing directory: $path entries: ${dirContents.length}');
...
// Method to retrieve a file from the server
  Future<void> retrieveFile(String filename) async {
    ...
    try {
      ...
      if (fileOperations.exists(filename)) {
        final fileStream = fileOperations.openRead(filename);
...
// Method to store a file on the server
  Future<void> storeFile(String filename) async {
    ...
    try {
      ...
      await fileOperations.writeFromStream(
        filename,
        dataSocket!.asBroadcastStream(
      ...
      if (transferInProgress) {
        ...
        await _closeDataSocket();
        sendResponse('226 Transfer complete');
        logger.generalLog('File transfer complete: $filename');
      }
    } catch (e) {
      logger.generalLog('Exception in storeFile: $e');
      sendResponse('550 Error writing file: $e');
      transferInProgress = false;
      await _closeDataSocket();
    }
  }
...
// Method to abort a file transfer
  ...
  Future<void> handleMlsd(String argument, FtpSession session) async {
    ...
    try {
      transferInProgress = true;
      final dirContents = await fileOperations.listEntries(argument);
      ...
      for (final entity in dirContents) {
        ...
        try {
          final facts = _formatMlsdFacts(entity);
...
// Method to abort a file transfer
  String _formatMlsdFacts(FtpFileEntry entry) {
    String type = entry.isDirectory ? "dir" : "file";
    String modify = DateFormat("yyyyMMddHHmmss")
        .format(entry.modified.toUtc()); // Use UTC time
  ...
  Future<void> handleMdtm(String argument, FtpSession session) async {
    try {
      ...
      final modifiedAt = await fileOperations.modificationTime(argument);
      final modificationTime = _formatMdtmTimestamp(modifiedAt);
